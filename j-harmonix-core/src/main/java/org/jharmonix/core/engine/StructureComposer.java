...
import org.jharmonix.core.engine.ChordSelector.HarmonicFunction;

import java.util.ArrayList;
import java.util.List;

/**
 * Assembles the full song form by composing individual section progressions.
 *
 * <p>The composer interprets the song-form string (e.g. {@code "AABA"},
 * {@code "verse-chorus-verse-bridge-chorus"}) and generates a coherent
 * section-by-section harmonic progression.
 *
 * <h3>Supported form tokens</h3>
 * <ul>
 *   <li>{@code A} / {@code verse}   – main verse section (8 bars, ends with tonic or turnaround)</li>
 *   <li>{@code B} / {@code bridge}  – contrasting section (8 bars, often modulates to IV or dominant)</li>
 *   <li>{@code C} / {@code chorus}  – chorus (same harmonic language as A, more conclusive)</li>
 *   <li>{@code outro}               – brief conclusive section (4 bars)</li>
 * </ul>
 *
 * <p>Unrecognised tokens default to a standard verse treatment.
 */
public final class StructureComposer {

    // Default number of chords per section (4/4 time, one chord per bar assumed)
    ...
    /**
     * Builds the full list of section progressions for the given request.
     *
     * @param request the generation parameters
     * @return ordered list of progressions, one per song-form section
     */
    public List<Progression> compose(ProgressionRequest request) {
        ...
        for (String token : tokens) {
            ...
            if (!bridge.isEmpty()) {
                // Prepend bridge chords into the section
    ...
    private Progression buildSection(String label, KeySignature key, int bars,
        ...
        // Simple functional pattern for 8 bars: T-SD-D-T-T-SD-D-T
    ...
    /**
     * Determines the key to use for the given section token. The Bridge section
     * typically modulates to the subdominant or dominant.
     */
    private KeySignature resolveKeyForSection(String token, KeySignature homeKey,
        ...
        return switch (token.toLowerCase()) {
            case "b", "bridge" -> homeKey.subdominantKey(); // IV area for bridge
            default            -> homeKey;
        };
    }
    ...
    /**
     * Splits the song-form string into section tokens.
     * Both {@code "AABA"} and {@code "verse-chorus-bridge"} formats are accepted.
     */
    private static String[] parseSongForm(String form) {
        ...
        // If every character is an uppercase letter assume compact notation (AABA)
        ...
        // Otherwise split on dashes / underscores / spaces
